...
my $alignmentInfile = "$tmpdir/$$.tabular";
...
END
		...
		close (STDOUT); # put the script run in backgroud
		...
		while(<ALN>)
		{
			...
			if (exists $queryId->{$hit[0]} && exists $subjectId->{$hit[1]})
			{
				...
				my $insertAlignmentA=$dbh->prepare("INSERT INTO alignment VALUES (NULL, '$alignEngine', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
				...
				my $insertAlignmentB=$dbh->prepare("INSERT INTO alignment VALUES (NULL, '$alignEngine', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
