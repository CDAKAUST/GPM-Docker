...
END
		...
		my $queryFile = "$tmpdir/$queryGenomeId.$$.seq";
		my $sequenceLength;
		...
		{
			$subjectFile = "$tmpdir/$subjectGenomeId.$$.seq";
		...
		if($alignEngine eq 'blastn')
		{
			...
			else
			{
				 ...
				 open (CMD,"$alignEngineList->{$alignEngine} -query $queryFile -task $task -db $subjectFile -evalue 1e-200 -perc_identity $identityAlignment -num_threads 4 -outfmt 6 |") or die "can't open CMD: $!";
				 #open (CMD,"$alignEngineList->{$alignEngine} -query $queryFile -task $task -db $subjectFile -evalue 1e-200 -perc_identity $identityAlignment -num_threads 4 -outfmt 6 -out /tmp/blast.out |") or die "can't open CMD: $!";
			}
		}
		else
		{
		...
		#my @list = <CMD>;
	  #open(FH, '>/tmp/cmd_blast.out') or die $!;
    ...
    my @Array_CMD=split "\n",<CMD>;
		...
		foreach(@Array_CMD)
		{
			...
			#my @hit = split("\s+",$_);
			#print "@hit";
			#my @hit2 = grep(s/\s*$//g, @hit);
			### Check @hit content.
			open(FH, '>/www/htdocs/data/tmp/LookErr.txt' ) or die $!;
			print FH join "|", @hit;
	    print FH "\n Text_Mark\n ";
			close(FH);

			next if($hit[0] eq $hit[1]); ### SOMETHING WRONG HERE
			...
			if($speedyMode)
			{
				...
				#write to alignment#### fix HERE                                  ''
				my $insertAlignmentA=$dbh->prepare("INSERT INTO alignment VALUES (NULL, '$alignEngine\_1e-200\_$identityAlignment\_$minOverlapAlignment', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
				$insertAlignmentA->execute(@hit);
				#$insertAlignmentA->execute($hit[0],$hit[1],$hit[2],$hit[3],$hit[4],$hit[5],$hit[6],$hit[7],$hit[8],$hit[9],$hit[10],$hit[11]);
				...
				my $insertAlignmentB=$dbh->prepare("INSERT INTO alignment VALUES (NULL, '$alignEngine\_1e-200\_$identityAlignment\_$minOverlapAlignment', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
				$insertAlignmentB->execute(@hit);
				#$insertAlignmentB->execute($hit[0],$hit[1],$hit[2],$hit[3],$hit[4],$hit[5],$hit[6],$hit[7],$hit[8],$hit[9],$hit[10],$hit[11]);
			}
			else
			{
				my $rerunBlastTwo = 0;
				...
				if($rerunBlastTwo)
				{
					...
					open (CMDA,"$alignEngineList->{'blastn'} -query $tmpdir/$hit[0].$$.seq -subject $tmpdir/$hit[1].$$.seq -dust no -evalue 1e-200 -perc_identity $identityAlignment -outfmt 6 |") or die "can't open CMD: $!";
          my @Array_CMDA= split "\n", <CMDA>;
					#while(<CMDA>)
					foreach (@Array_CMDA)
					{
					...
					if($goodOverlap)
					{
						foreach (@alignments)
						{
							my @detailedHit = split("\t",$_);
							#write to alignment
							my $insertAlignment=$dbh->prepare("INSERT INTO alignment VALUES (NULL, '$alignEngine\_1e-200\_$identityAlignment\_$minOverlapAlignment', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
							$insertAlignment->execute(@detailedHit);
							#$insertAlignment->execute($detailedHit[0],$detailedHit[1],$detailedHit[2],$detailedHit[3],$detailedHit[4],$detailedHit[5],$detailedHit[6],$detailedHit[7],$detailedHit[8],$detailedHit[9],$detailedHit[10],$detailedHit[11]);
						}
					}
				}
			}
		...
		`rm $tmpdir/*.aln.html`; #delete cached files
