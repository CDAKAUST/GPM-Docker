...
END
		...
		if($goodOverlap)
		{
			foreach (@alignments)
			{
				my @hit = split("\t",$_);
				#write to alignment
				my $insertAlignment=$dbh->prepare("INSERT INTO alignment VALUES (NULL, 'SEQtoSEQ_1e-200_$identityBlast\_$minOverlapBlast', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
...
END
			...
			while(<CMD>)
			{
				...
				if($speedyMode)
				{
					...
					my $insertAlignmentA=$dbh->prepare("INSERT INTO alignment VALUES (NULL, 'SEQtoSEQ\_1e-200\_$identityBlast\_$minOverlapBlast', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
					...
					my $insertAlignmentB=$dbh->prepare("INSERT INTO alignment VALUES (NULL, 'SEQtoSEQ\_1e-200\_$identityBlast\_$minOverlapBlast', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
					...
					if($rerunBlastTwo)
					{
						...
						if($goodOverlap)
						{
							foreach (@alignments)
							{
								my @detailedHit = split("\t",$_);
								#write to alignment
								my $insertAlignment=$dbh->prepare("INSERT INTO alignment VALUES (NULL, 'SEQtoSEQ\_1e-200\_$identityBlast\_$minOverlapBlast', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
